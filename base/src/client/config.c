...
int load_client_config(client_config_t* config) {
    ...
    if (!file) {
        ...
        return save_client_config(config); // 保存默认配置
    }
...
// 无参数版本的load_config，使用全局配置
int load_config() {
    return load_client_config(&g_client.config);
}

// 无参数版本的save_config，使用全局配置
int save_config() {
    return save_client_config(&g_client.config);
}

// 设置默认配置
void set_default_config() {
    strncpy(g_client.config.server_host, DEFAULT_SERVER_HOST, sizeof(g_client.config.server_host) - 1);
    g_client.config.server_host[sizeof(g_client.config.server_host) - 1] = '\0';
    g_client.config.server_port = DEFAULT_SERVER_PORT;
    g_client.config.auto_update = 1;
    g_client.config.heartbeat_enabled = 1;
    g_client.config.heartbeat_interval = DEFAULT_HEARTBEAT_INTERVAL;
    g_client.config.connection_timeout = DEFAULT_CONNECTION_TIMEOUT;
    g_client.config.max_retry_count = DEFAULT_MAX_RETRY;
    g_client.config.log_level = 1;
    printf("配置已设置为默认值\n");
}