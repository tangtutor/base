...
int handle_version_response(version_response_msg_t* response) {
    ...
    switch (response->status) {
        ...
        case STATUS_UPDATE_AVAILABLE:
            ...
            // 检查是否为GUI模式，GUI模式下不自动更新
            extern int gui_mode_active; // 声明外部变量
            if (gui_mode_active) {
                printf("GUI模式下发现新版本，请手动更新\n");
                log_message_to_gui("发现新版本 %s，请手动更新", response->latest_version);
            } else if (g_client.config.auto_update) {
...
int apply_update() {
    ...
    if (len != -1) {
        ...
        snprintf(script_path, sizeof(script_path), "%supdate_script.sh", UPDATE_DIR);
        ...
        fprintf(script_file, "sleep 2\n");
        fprintf(script_file, "cp %s %s\n", new_client_path, current_exe);
        fprintf(script_file, "chmod +x %s\n", current_exe);
        fprintf(script_file, "cd %s\n", "/home/lt/base");
        fprintf(script_file, "%s --gui &\n", current_exe);
